...
// A M2) and FLCM (MENA, Group A M3). Seeds use the DB era codes (team_era_name),
// since winner attribution and logo lookup match on those. True Rippers is "TRX"
// (team_era_name.era_code was renamed from "TR" to match the bracket art, 2026-07-06).
...
export function resolveMainGroup(groupKey, mainSeries = []) {
  ...
  const groupSeries = mainSeries
    .filter((s) => s.stage !== 'Knockout' && teamSet.has(s.team_a) && teamSet.has(s.team_b))
    .sort((a, b) =>
