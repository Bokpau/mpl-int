...
async function get(path) {
  const headers = isServer && process.env.INTERNAL_API_KEY
    ...
    : {};
  const res = await fetch(`${API}${path}`, { next: { revalidate: 300, tags: ['intl'] }, headers });
  if (!res.ok) throw new Error(`API error: ${res.status}`);
  return res.json();
}

export const api = {
  editions:    (q = '')     => get(`/api/intl/editions${q}`),
  accolades:   (q = '')     => get(`/api/intl/accolades${q}`),
  standings:   (q = '')     => get(`/api/intl/standings${q}`),
  overview:    (q = '')     => get(`/api/intl/overview${q}`),
  ...
  heroDraftStats:(heroid, q = '') => get(`/api/intl/heroes/${encodeURIComponent(heroid)}/draft-stats${q}`),
  heroGames:     (heroid, q = '') => get(`/api/intl/heroes/${encodeURIComponent(heroid)}/games${q}`),
