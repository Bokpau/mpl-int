...
export function effectiveFilters(sp = {}, featured = null) {
  ...
  const division = rawDiv === 'female' || rawDiv === 'women' ? 'female' : 'open';

  const rawScope = String(sp.scope || '').toUpperCase();
  let scope = '';
  if (division === 'female') {
    scope = rawScope === 'MWI' ? rawScope : '';
  } else {
    scope = rawScope === 'MSC' || rawScope === 'MWC' ? rawScope : '';
  }
  ...
  if (isAll) {
    ...
    const isMwiSeason = rawSeason.toUpperCase().includes('MWI');
    if ((division === 'female' && isMwiSeason) || (division === 'open' && !isMwiSeason)) {
      season = rawSeason; // a specific edition was chosen and matches the division
    }
  } else if (featured && featured.season) {
    const isMwiFeatured = String(featured.season).toUpperCase().includes('MWI');
    if ((division === 'female' && isMwiFeatured) || (division === 'open' && !isMwiFeatured)) {
...
// ── Schedule rows vs the stage filter ───────────────────────────────────────
// intl_schedule rows carry a `phase` ("Wild Card" / "Group Stage" / "Knockout")
// but no stage_type, and /api/intl/schedule ignores the `stage` param — so a
// stage-filtered page gets the WHOLE edition's schedule back while its match
// rows are stage-scoped. Filter schedule rows here or already-played series from
// the other stage look like upcoming fixtures (their match_codes are missing
// from the stage-scoped played set). Wild Card = qualifier; Group Stage and
// Knockout are both Main.
export function scheduleInStage(row, stage) {
  if (stage !== 'main' && stage !== 'qualifier') return true;
  const isWildCard = String(row?.phase || '').toLowerCase().includes('wild');
  return stage === 'qualifier' ? isWildCard : !isWildCard;
}
...
export const familyLabel = (code) =>
  code === 'MWC' ? 'M-Series' : code === 'MSC' ? 'MSC' : code === 'MWI' ? 'MWI' : code || '';

// A single edition's title, e.g. "MSC 2025" / "M-Series M6".
export function editionTitle(edition) {
  if (!edition) return '';
  const fam = familyLabel(edition.tournament_code);
  const season = String(edition.season ?? '').trim();
  // MSC season labels already include "MSC" (e.g. "MSC 2026") — don't double the
  // family prefix. M-Series labels are "M1".."M7", so they still get the prefix.
  if (fam && season.toUpperCase().startsWith(fam.toUpperCase())) return season;
  return `${fam} ${season}`.trim();
}
