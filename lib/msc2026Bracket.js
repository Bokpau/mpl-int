...
export function buildSeries(matches) {
  ...
  for (const s of out) {
    // Required wins = majority of the series length. match_count arrives as a STRING
    // ("BO5") from /matches/rich and as undefined from /matches, so parse the digits
    // out rather than dividing it — Math.ceil('BO5' / 2) is NaN, which silently makes
    // every series undecided and wipes winners off the whole bracket.
    const boNum = Number(String(s.match_count ?? '').replace(/[^0-9]/g, '')) || 0;
    const reqWins = boNum
      ? Math.ceil(boNum / 2)
      : (s.stage === 'Knockout' ? 3 : (s.games <= 1 ? 1 : 2));
    // A series isn't over just because one side leads — only call a winner once a side
    // has clinched, or an in-progress Bo5 leaks a false winner into the SF/3rd/GF slots.
    const decided = (s.a_wins >= reqWins || s.b_wins >= reqWins) && s.a_wins !== s.b_wins;
