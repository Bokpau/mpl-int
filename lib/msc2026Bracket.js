...
export function buildSeries(matches) {
  ...
  for (const m of matches) {
    ...
    if ((wKey && s.team_a_key && wKey === s.team_a_key) || (wEra && (wEra === s.team_a || wEra === m.team_a))) s.a_wins++;
    else if ((wKey && s.team_b_key && wKey === s.team_b_key) || (wEra && (wEra === s.team_b || wEra === m.team_b))) s.b_wins++;
    else if (wCamp === 1) s.a_wins++;
    else if (wCamp === 2) s.b_wins++;
    if (String(m.played_at || '') > String(s.played_at || '')) s.played_at = m.played_at;
  }
  const out = [...byMatch.values()];
  for (const s of out) {
    // Determine required wins: match_count majority (e.g. Bo5 -> 3, Bo7 -> 4), or 3 for Knockout, 2 for Bo3.
    const reqWins = s.match_count ? Math.ceil(s.match_count / 2) : (s.stage === 'Knockout' ? 3 : 2);
    const decided = s.a_wins >= reqWins || s.b_wins >= reqWins || s.a_wins !== s.b_wins;
    s.winner_code = s.a_wins > s.b_wins ? s.team_a : s.b_wins > s.a_wins ? s.team_b : null;
