...
export function resolveKnockoutBracket(mainSeries = [], qualifiersA = [], qualifiersB = []) {
  const knockoutSeries = mainSeries.filter((s) => s.stage === 'Knockout' || (s.match_code && s.match_code.startsWith('MSC202607')) || s.match_count >= 5);
  const used = new Set();
  ...
  for (const spec of KNOCKOUT_NODES) {
    const seededA = getSeeded(spec, 'a');
    const seededB = getSeeded(spec, 'b');
    const rawA = spec.seedA ? seededA : resolveRef(spec.a);
    const rawB = spec.seedB ? seededB : resolveRef(spec.b);
    const a = rawA || (spec.seedA ? spec.defaultA : null);
    const b = rawB || (spec.seedB ? spec.defaultB : null);
    const aLabel = rawA || spec.defaultA;
    const bLabel = rawB || spec.defaultB;
